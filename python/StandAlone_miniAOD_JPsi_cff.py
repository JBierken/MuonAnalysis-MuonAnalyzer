...
muon = cms.EDAnalyzer('StandAloneMuonMiniAODAnalyzer',
          ...
          # tracks                   = cms.InputTag("generalTracks"),
           ...
           #magneticfield           = cms.ESInputTag("moduleLabel", "productLabel"),
           ...
           probeSelectionSA         = cms.string("pt()>0"),     # string for probe
           muonOnly                 = cms.bool(False),          # allow only reco/pat Muon for probes
           probeMuonSelection       = cms.string("pt()>0"),     # string for probe (reco or pat Muon)
           pairMassMin              = cms.double(2.0),          # min mass of mu pair
           pairMassMax              = cms.double(4.0),          # max mss of mu pair (9999.0 for high mass C&C)
           ...
           maxRelPtProbeTrkMuon     = cms.double(5.0),          # max [pt(mu)-pt(trk)]/pt(trk) for probe/offline
           maxRelPtProbeTrkSAMuon   = cms.double(5.0),          # max [pt(SAmu)-pt(trk)]/pt(trk) for probe/offline
           ...
           maxEtaProbeTrkMuon       = cms.double(0.3),          # max ETA for probe/offline
           maxEtaProbeTrkSAMuon     = cms.double(0.15),         # max ETA for trk / SA muon
           minPtTrkSA               = cms.double(0),            # min Pt for trk matching SA muons
           momPdgId                 = cms.uint32(443),
           genRecoDrMatch           = cms.double(0.03),
           saveStandAloneTree       = cms.bool(True),
           saveTnPTree              = cms.bool(True),          
           Zresonance               = cms.bool(False),
           debug                    = cms.int32(1),
