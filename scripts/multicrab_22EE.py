...
def getOptions():
    ...
    parser.add_option('-w', '--workArea',
                      dest = 'workArea',
                      default = 'crab_standAloneMuons_22EE',
    ...
    parser.add_option('-f', '--configFile',
                      dest = 'configFile',
                      default = os.path.join(os.environ['CMSSW_BASE'], 'src/MuonAnalysis/MuonAnalyzer/test/run_muonAnalyzer_cfg_22EE.py'),
...
def main():
    ...
    if crabCmd == 'submit':
        ...
        if storageSite == 'FNAL':
            ...
            config.Data.outLFNDirBase = '/store/group/phys_tracking/%s/FinalJetscrab_AOD_Run2022EE_pTFromTracker/%s/%s/%s/%s' % (getUsername(), particle, resonance, era, dataTier)
            ...
            config.Data.outLFNDirBase = '/store/user/%s/TnP_ntuples/All_tracks/%s/%s/%s/%s' % (getUsername(), particle, resonance, era, dataTier)
