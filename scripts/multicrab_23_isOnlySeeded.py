...
def getOptions():
    ...
    parser.add_option('-f', '--configFile',
                      dest = 'configFile',
                      default = os.path.join(os.environ['CMSSW_BASE'], 'src/MuonAnalysis/MuonAnalyzer/test/run_muonAnalyzer_cfg_23_isOnlySeeded.py'),
...
def main():
    ...
    if crabCmd == 'submit':
        ...
        if storageSite == 'FNAL':
            ...
            config.Data.outLFNDirBase = '/store/group/phys_tracking/%s/FinalJetscrab_AOD_Run2023_pTFromTracker_isOnlySeeded/%s/%s/%s/%s' % (getUsername(), particle, resonance, era, dataTier)
